...
const BASES: Record<HeroClass, Build> = {
  warrior: { classId: "warrior", name: "Guerreiro", attackName: "Corte de Ferro", trait: "Vida e armadura superiores. Domina o espaço próximo com golpes pesados e atravessa grupos. Começa com uma onda de impacto a cada 6,5 s.", accent: "#e2a95e", maxHp: 200, hp: 200, moveSpeed: 146, damage: 39, attackRange: 165, projectileSpeed: 490, attackInterval: .6, projectileCount: 1, spread: 0, pierce: 3, armor: 4, critChance: .08, critDamage: 1.7, magnet: 96, xpGain: 1, luck: 0, regen: .25, area: 1.05, fireballInterval: 0, rageInterval: 6.5, slowFieldInterval: 0, multishotEvery: 0, ultimate: null, ultimateName: "Nenhuma ultimate", ultimateCharge: 0, ultimateMax: 100, ultimateLevel: 0, dashMax: 1, dashCooldown: 5, dashDamage: 0, dashClone: false, dashShield: .55, ...boonDefaults() },
  ...
  mage: { classId: "mage", name: "Mago", attackName: "Seta Arcana", trait: "Pouca vida, grande dano e controle do campo. Começa lançando um orbe explosivo fortalecido a cada 5,5 s.", accent: "#9f8cf0", maxHp: 116, hp: 116, moveSpeed: 173, damage: 44, attackRange: 580, projectileSpeed: 500, attackInterval: .78, projectileCount: 1, spread: 0, pierce: 1, armor: 0, critChance: .1, critDamage: 1.8, magnet: 104, xpGain: 1, luck: .02, regen: .1, area: 1.15, fireballInterval: 5.5, rageInterval: 0, slowFieldInterval: 0, multishotEvery: 0, ultimate: null, ultimateName: "Nenhuma ultimate", ultimateCharge: 0, ultimateMax: 100, ultimateLevel: 0, dashMax: 1, dashCooldown: 5, dashDamage: 0, dashClone: false, dashShield: .55, ...boonDefaults() },
};

const SHOP_ITEMS: ShopItem[] = [
  { id: "vitality", name: "Coração Gravado", description: "Comece cada tentativa com mais vida máxima.", effect: "+4% de vida por nível", maxRank: 5, costs: [4, 7, 10, 14, 19], icon: Heart, apply: (b, rank) => { b.maxHp = Math.round(b.maxHp * (1 + rank * .04)); } },
  { id: "power", name: "Fio da Lâmina", description: "Fortalece todo o dano desde o início da tentativa.", effect: "+3,5% de dano por nível", maxRank: 5, costs: [5, 8, 11, 15, 20], icon: Swords, apply: (b, rank) => { b.damage *= 1 + rank * .035; } },
  { id: "magnet", name: "Ímã de Ecos", description: "Puxa fragmentos de experiência de mais longe.", effect: "+20 de coleta por nível", maxRank: 5, costs: [3, 5, 8, 11, 15], icon: Orbit, apply: (b, rank) => { b.magnet += rank * 20; } },
  { id: "memory", name: "Memória Fluida", description: "Transforma fragmentos recolhidos em mais XP.", effect: "+3,5% de XP por nível", maxRank: 5, costs: [5, 8, 11, 15, 20], icon: Sparkles, apply: (b, rank) => { b.xpGain *= 1 + rank * .035; } },
  { id: "fortune", name: "Fio da Fortuna", description: "Inclina as ofertas para raridades superiores.", effect: "+1% de sorte por nível", maxRank: 5, costs: [6, 9, 13, 18, 24], icon: Crown, apply: (b, rank) => { b.luck += rank * .01; } },
  { id: "recovery", name: "Bálsamo Persistente", description: "Recupera vida continuamente durante o combate.", effect: "+0,08 PV/s por nível", maxRank: 5, costs: [4, 7, 10, 14, 19], icon: HeartPulse, apply: (b, rank) => { b.regen += rank * .08; } },
];
...
const CARDS: UpgradeCard[] = [
  ...
  normal({ id: "dash-flow", name: "Fluxo de Esquiva", description: "Reduz em 22% a recarga da esquiva.", classId: "general", apply: b => { b.dashCooldown = Math.max(1.65, b.dashCooldown * .78); } }),
  normal({ id: "dash-impact", name: "Passagem Violenta", description: "A esquiva causa 175% do seu dano aos inimigos atravessados.", classId: "general", apply: b => { b.dashDamage += 1.75; } }),
  ...
  rare({ id: "dash-clone", name: "Imagem Residual", description: "A esquiva deixa um eco mais forte que explode e distrai perseguidores.", classId: "general", apply: b => { b.dashClone = true; b.dashDamage += 1; } }),
  ...
  legendary({ id: "perfect-dash", name: "Passo Fora do Tempo", description: "Esquiva recarrega em 1,5 s, ganha uma carga e causa dano elevado.", classId: "general", apply: b => { b.dashCooldown = 1.5; b.dashMax += 1; b.dashDamage += 1.6; } }),

  normal({ id: "war-steel", name: "Aço Temperado", description: "+2 de armadura e +11% de dano.", classId: "warrior", apply: b => { b.armor += 2; b.damage *= 1.11; } }),
  normal({ id: "war-arc", name: "Arco da Lâmina", description: "+30% de alcance e +28% de área dos golpes.", classId: "warrior", apply: b => { b.attackRange *= 1.3; b.area *= 1.28; } }),
  normal({ id: "war-rage", name: "Fúria Cadenciada", description: "A onda de impacto fica 10% maior e ocorre cerca de 22% mais vezes.", classId: "warrior", apply: b => { b.rageInterval = b.rageInterval ? Math.max(2.2, b.rageInterval * .78) : 4.5; b.area *= 1.1; } }),
  normal({ id: "war-speed", name: "Golpe Repetido", description: "+20% de velocidade de ataque.", classId: "warrior", apply: b => { b.attackInterval *= .8; } }),
  rare({ id: "war-counter", name: "Geometria de Guerra", description: "+34% de dano e +4 de perfuração.", classId: "warrior", apply: b => { b.damage *= 1.34; b.pierce += 4; } }),
  ...
  rare({ id: "war-blood", name: "Lâmina Sanguínea", description: "+25% de dano e +1 de regeneração.", classId: "warrior", apply: b => { b.damage *= 1.25; b.regen += 1; } }),
  legendary({ id: "war-quake", name: "Lâmina de Terremoto", description: "Golpes ficam 100% maiores, atravessam +6 e causam +30% de dano.", classId: "warrior", apply: b => { b.area *= 2; b.pierce += 6; b.damage *= 1.3; } }),
  ...
  ult(rare({ id: "uw2", name: "ULT · Último Guardião", description: "Cura 55% e cria uma muralha destrutiva.", classId: "warrior", ultimate: "guardian", apply: b => { b.ultimate = "guardian"; b.ultimateName = "Último Guardião"; b.ultimateCharge = 0; b.ultimateLevel += 1; } })),
  ult(legendary({ id: "uw3", name: "ULT · Quebra-Mundo", description: "Quatro terremotos atravessam todo o campo.", classId: "warrior", ultimate: "earthbreaker", apply: b => { b.ultimate = "earthbreaker"; b.ultimateName = "Quebra-Mundo"; b.ultimateCharge = 0; b.ultimateLevel += 1; } })),

  normal({ id: "ar-draw", name: "Corda Tensa", description: "+20% de cadência e +15% de velocidade de flecha.", classId: "archer", apply: b => { b.attackInterval *= .8; b.projectileSpeed *= 1.15; } }),
  normal({ id: "ar-split", name: "Flecha Bipartida", description: "+1 flecha com dispersão, −4% de dano.", classId: "archer", apply: b => { b.projectileCount += 1; b.spread = Math.max(.08, b.spread); b.damage *= .96; } }),
  normal({ id: "ar-hunt", name: "Instinto do Caçador", description: "+16% crítico e +12% movimento.", classId: "archer", apply: b => { b.critChance += .16; b.moveSpeed *= 1.12; } }),
  ...
  rare({ id: "ar-ghost", name: "Ponta Fantasma", description: "+4 perfuração e +23% de dano.", classId: "archer", apply: b => { b.pierce += 4; b.damage *= 1.23; } }),
  rare({ id: "ar-focus", name: "Olho Imóvel", description: "+30% crítico e críticos causam 250%.", classId: "archer", apply: b => { b.critChance += .3; b.critDamage = Math.max(2.5, b.critDamage); } }),
  legendary({ id: "ar-stars", name: "Constelação de Setas", description: "+4 flechas, +28% de cadência e +3 perfuração.", classId: "archer", apply: b => { b.projectileCount += 4; b.spread = Math.max(.1, b.spread); b.attackInterval *= .72; b.pierce += 3; } }),
  ...
  normal({ id: "ma-fire", name: "Bola de Fogo", description: "A explosão fica 12% maior e é conjurada cerca de 22% mais vezes.", classId: "mage", apply: b => { b.fireballInterval = b.fireballInterval ? Math.max(2.2, b.fireballInterval * .78) : 4.5; b.area *= 1.12; } }),
  normal({ id: "ma-slow", name: "Poço de Lentidão", description: "A zona fica 12% maior e é criada cerca de 20% mais vezes.", classId: "mage", apply: b => { b.slowFieldInterval = b.slowFieldInterval ? Math.max(3.4, b.slowFieldInterval * .8) : 7; b.area *= 1.12; } }),
  normal({ id: "ma-focus", name: "Foco Prismático", description: "+25% de dano e +28% de área.", classId: "mage", apply: b => { b.damage *= 1.25; b.area *= 1.28; } }),
  normal({ id: "ma-speed", name: "Conjuração Abreviada", description: "+20% de velocidade de ataque.", classId: "mage", apply: b => { b.attackInterval *= .8; } }),
  rare({ id: "ma-chain", name: "Geometria Encadeada", description: "+2 projéteis, +3 perfuração e +10% de cadência.", classId: "mage", apply: b => { b.projectileCount += 2; b.spread = Math.max(.12, b.spread); b.pierce += 3; b.attackInterval *= .9; } }),
  rare({ id: "ma-collapse", name: "Colapso Arcano", description: "+58% de dano, −15% de vida máxima.", classId: "mage", apply: b => { b.damage *= 1.58; b.maxHp *= .85; b.hp = Math.min(b.hp, b.maxHp); } }),
  rare({ id: "ma-field", name: "Campo Persistente", description: "Áreas ficam 55% maiores e +1,3 de regeneração.", classId: "mage", apply: b => { b.area *= 1.55; b.regen += 1.3; } }),
  legendary({ id: "ma-star", name: "Estrela Engarrafada", description: "Explosões +90%, +5 perfuração e +30% de dano.", classId: "mage", apply: b => { b.area *= 1.9; b.pierce += 5; b.damage *= 1.3; } }),
...
function applyGodBoon(god: GodId, slot: BoonSlot, b: Build, p: number) {
  if (god === "wild") return;
  if (slot === "attack") {
    b.attackGod = god; b.damage *= 1 + .06 * p; b.projectileSize *= 1 + .04 * p;
    ...
    b.dashGod = god; b.dashCooldown = Math.max(1.35, b.dashCooldown * (1 - .065 * p)); b.dashDamage += .45 * p; b.dashTrail = true;
    ...
    b.technique1God = god;
    if (god === "flame" || god === "storm" || god === "venom") b.fireballInterval = Math.max(1.9, b.fireballInterval ? b.fireballInterval * (1 - .075 * p) : 6.6 - .6 * p);
    else if (god === "frost" || god === "tide") b.slowFieldInterval = Math.max(2.9, b.slowFieldInterval ? b.slowFieldInterval * (1 - .07 * p) : 7.8 - .55 * p);
    else if (["gale", "shadow", "radiance", "blood", "moon", "sun"].includes(god)) { b.novaInterval = Math.max(2.5, b.novaInterval ? b.novaInterval * (1 - .07 * p) : 7.3 - .6 * p); b.area *= 1 + .055 * p; }
    if (god === "stone") { b.trapInterval = Math.max(2.4, 6.8 - .65 * p); b.trapDamage += 1 * p; }
    ...
    b.technique2God = god;
    b.orbitals += Math.max(2, Math.round(1 + p / 1.2)); b.orbitalDamage += .1 * p;
    b.orbitalRadius += 10 + 6 * p;
    b.projectileSize *= 1 + .035 * p;
  ...
  if (b.classId === "warrior") { if (slot === "attack") b.damage *= 1 + .03 * p; if (slot === "technique2") b.armor += Math.floor(p / 2); }
  else if (b.classId === "archer") { if (slot === "attack") b.projectileSpeed *= 1 + .035 * p; if (slot === "dash") b.dashCooldown *= 1 - Math.min(.12, .025 * p); }
  else { if (slot === "attack" || slot === "technique1") b.area *= 1 + .035 * p; if (slot === "technique2") b.orbitalDamage += .05 * p; }
}
...
function boonDescription(god: GodId, slot: BoonSlot, rarity: Rarity) {
  ...
  if (slot === "attack") {
    const effects: Record<GodId, string> = {
      flame: `Ataques causam +${pct(14)}% de dano e podem incendiar. A queimadura consome ${pct(10)}% da vida máxima do alvo em 3,5 s; novos acertos renovam o fogo.`,
      tide: `Ataques causam +${pct(6)}% de dano, atravessam +${Math.ceil(p)} alvo(s) e empurram inimigos.`,
      gale: `Ataques causam +${pct(6)}% de dano, viajam +${pct(5)}% mais rápido e aumentam seu movimento em ${pct(8)}%; a partir de Incomum, dispara +1 projétil.`,
      frost: `Ataques causam +${pct(6)}% de dano, aplicam 28% de lentidão e passam a criar periodicamente uma zona congelante.`,
      shadow: `Ataques causam +${pct(6)}% de dano, ganham +${pct(4.5)}% de crítico e +${pct(3.5)}% de chance de executar com uma explosão sombria.`,
      radiance: `Ataques causam +${pct(6)}% de dano e sua cadência aumenta em aproximadamente ${pct(7)}%.`,
      ...
      moon: `Ataques causam +${pct(6)}% de dano, ficam +${pct(8)}% maiores e recebem perfuração nas raridades superiores.`,
  ...
  if (slot === "dash") {
    ...
    return `A esquiva recarrega ${pct(6.5)}% mais rápido, causa +${(0.45 * p).toFixed(1)}× de dano-base ao atravessar inimigos e ${extras[god]}.` + (god === "blood" ? " Parte do poder cobra vida ao atacar." : "");
  }
  if (slot === "technique1") {
    const effects: Record<GodId, string> = {
      flame: `A cada ${(6.6 - .6 * p).toFixed(1)} s, lança um meteoro de 225% do dano, grande explosão e queimadura garantida.`,
      tide: `A cada ${(7.8 - .55 * p).toFixed(1)} s, ergue uma maré por 5,4 s que freia ameaças; periodicamente você fica intangível por ${(0.45 + .16 * p).toFixed(1)} s.`,
      gale: `A cada ${(7.3 - .6 * p).toFixed(1)} s, uma rajada circular de 165% do dano abre espaço ao redor do personagem.`,
      frost: `A cada ${(7.8 - .55 * p).toFixed(1)} s, cria um inverno local por 5,4 s que reduz inimigos e projéteis em 50%.`,
      shadow: `A cada ${(7.3 - .6 * p).toFixed(1)} s, uma fenda causa 165% do dano; cada morte tem uma chance de 0,0${Math.max(1, Math.round(.8 * p))}% de conceder um nível inteiro.`,
      radiance: `A cada ${(7.3 - .6 * p).toFixed(1)} s, um halo causa 165% do dano; fragmentos valem mais e alguns inimigos derrubam 3× XP.`,
      venom: `A cada ${(6.6 - .6 * p).toFixed(1)} s, semeia um jardim cáustico fortalecido que aplica 2 acúmulos e continua envenenando quem permanece nele.`,
      storm: `A cada ${(6.6 - .6 * p).toFixed(1)} s, dispara três relâmpagos bifurcados fortalecidos, rápidos e capazes de saltar entre alvos.`,
      blood: `A cada ${(7.3 - .6 * p).toFixed(1)} s, uma maré rubra marca todos ao redor; ao atingir ${Math.max(4, 9 - Math.floor(p))} marcas, eles explodem.`,
      stone: `A cada ${(6.8 - .65 * p).toFixed(1)} s, arma uma runa tectônica fortalecida no chão. Ela espera um inimigo e então explode em grande área.`,
      moon: `A cada ${(7.3 - .6 * p).toFixed(1)} s, um eclipse causa 165% do dano em grande área e aproxima fragmentos distantes.`,
      sun: `A cada ${(7.3 - .6 * p).toFixed(1)} s, uma coluna solar fortalecida marca e incinera uma área; inimigos com três marcas explodem.`,
      wild: `Guerra aprimora uma técnica já equipada; não cria uma técnica elemental própria.`,
    }; return effects[god];
  }
  if (slot === "technique2") {
    const count = god === "wild" ? Math.max(5, Math.floor(p / 2)) : Math.max(2, Math.round(1 + p / 1.2));
    const radiusGain = Math.round(10 + 6 * p);
...
function classChoices(g: Game) {
  const highestRank = g.level < 4 ? 1 : g.level < 7 ? 3 : 5;
  const classPool = CARDS.filter(card => card.classId === g.build.classId && RARITY_RANK[card.rarity] <= highestRank).sort(() => Math.random() - .5);
  const generalPool = CARDS.filter(card => card.classId === "general" && RARITY_RANK[card.rarity] <= highestRank).sort(() => Math.random() - .5);
  return [...classPool.slice(0, 2), ...generalPool.slice(0, 1)];
}
function choices(g: Game) {
  if (g.level === 2) return classChoices(g);
  if (g.level >= 3 && Math.random() < .065) return [...CURSES].sort(() => Math.random() - .5).slice(0, 3);
  if (g.level >= 4 && Math.random() < .2) return classChoices(g);
...
function castTechnique(g: Game) {
  ...
  if (god === "flame") projectile(g, g.player, angle, g.build.projectileSpeed * .82, g.build.damage * 2.25, false, { radius: 14 * g.build.projectileSize, area: 125 * g.build.area, color, shape: "ember", god, special: "technique" });
  else if (god === "storm") for (let i = -1; i <= 1; i++) projectile(g, g.player, angle + i * .09, g.build.projectileSpeed * 1.35, g.build.damage * 1.3, false, { radius: 5.5, pierce: 2, life: 2.2, color, shape: "shard", god, special: "technique" });
  else if (god === "venom") g.zones.push({ id: id(g), x: victim.x, y: victim.y, radius: 125 * g.build.area, life: 4.8, slow: .28, damage: g.build.damage * .42, tick: 0, color, hostile: false, god: "venom" });
}
...
function charge(g: Game, damage: number) {
  ...
  g.build.ultimateCharge = 0; const p = 1 + g.build.ultimateLevel * .18; const whole = Math.max(g.width, g.height);
  const god = g.build.ultimateGod;
  if (god) {
    if (god === "flame") { for (let i = 0; i < 18; i++) g.zones.push({ id: id(g), x: 45 + Math.random() * (g.width - 90), y: 45 + Math.random() * (g.height - 90), radius: 95 + Math.random() * 60, life: 3.6, slow: 0, damage: g.build.damage * 2.4 * p, tick: .25 + i * .045, color: GODS.flame.color, hostile: false, kind: "meteor", god: "flame" }); }
    else if (god === "tide") { g.build.hp = g.build.maxHp; g.player.invulnerable = 6; g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: whole, life: 6, slow: .75, damage: g.build.damage * .9 * p, tick: 0, color: GODS.tide.color, hostile: false, god: "tide" }); }
    else if (god === "gale") { g.player.invulnerable = 2.5; const aims: Point[] = g.phase === "boss" ? Array.from({ length: 55 }, () => g.boss) : g.enemies.slice(0, 70); aims.forEach((aim, i) => projectile(g, g.player, Math.atan2(aim.y - g.player.y, aim.x - g.player.x) + (i % 5 - 2) * .04, 1080, g.build.damage * 1.85 * p, false, { pierce: 5, life: 2.3, color: GODS.gale.color, shape: "shard", god: "gale", special: "technique" })); }
    else if (god === "frost") { g.player.invulnerable = 3.5; g.zones.push({ id: id(g), x: g.width / 2, y: g.height / 2, radius: whole, life: 8, slow: .94, damage: g.build.damage * .8 * p, tick: 0, color: GODS.frost.color, hostile: false, god: "frost" }); }
    else if (god === "shadow") { g.shadowCloneTime = 11 * g.build.effectDuration; g.player.invulnerable = 3; g.zones.push({ id: id(g), x: g.width / 2, y: g.height / 2, radius: whole * .76, life: 4.5, slow: .68, damage: g.build.damage * .85 * p, tick: 0, color: GODS.shadow.color, hostile: false, god: "shadow", fear: 2 }); }
    else if (god === "radiance") { g.player.invulnerable = 5; const aims: Point[] = g.phase === "boss" ? Array.from({ length: 50 }, () => g.boss) : Array.from({ length: 50 }, (_, i) => g.enemies[i % Math.max(1, g.enemies.length)] ?? g.boss); aims.forEach((aim, i) => projectile(g, g.player, Math.atan2(aim.y - g.player.y, aim.x - g.player.x) + Math.sin(i) * .05, 1600, g.build.damage * 1.3 * p, false, { pierce: 5, life: 1.5, color: GODS.radiance.color, shape: "ember", god: "radiance", special: "technique" })); }
    ...
    else if (god === "stone") { g.player.invulnerable = 8 * g.build.effectDuration; g.build.barrier = Math.max(g.build.barrier, g.build.maxHp * .5); g.build.barrierMax = Math.max(g.build.barrierMax, g.build.barrier); for (let i = 0; i < 12; i++) g.zones.push({ id: id(g), x: 50 + Math.random() * (g.width - 100), y: 50 + Math.random() * (g.height - 100), radius: 72 + Math.random() * 35, life: 3.1, slow: .35, damage: g.build.damage * 1.6 * p, tick: .18 + i * .055, color: GODS.stone.color, hostile: false, kind: "meteor", god: "stone" }); }
    else if (god === "moon") { g.moonEchoTime = 12 * g.build.effectDuration; g.player.invulnerable = 3.5; for (let i = 0; i < 42; i++) projectile(g, g.player, i * Math.PI * 2 / 42, 540 + (i % 3) * 100, g.build.damage * 1.8 * p, false, { pierce: 6, area: 32, life: 3.8, color: GODS.moon.color, shape: "orb", god: "moon", special: "technique" }); }
  ...
  if (g.build.ultimate === "war-cry") { g.player.invulnerable = 4; g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: 285, life: 4, slow: .6, damage: g.build.damage * 1 * p, tick: 0, color: "#f0ad55", hostile: false }); }
  else if (g.build.ultimate === "guardian") { g.build.hp = Math.min(g.build.maxHp, g.build.hp + g.build.maxHp * .55); g.player.invulnerable = 4; g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: 235, life: 4.5, slow: .35, damage: g.build.damage * .85 * p, tick: 0, color: "#f3dc99", hostile: false }); }
  else if (g.build.ultimate === "earthbreaker") for (let i = 0; i < 4; i++) g.zones.push({ id: id(g), x: g.width * (.15 + i * .23), y: g.player.y, radius: 205, life: 2.8, slow: .45, damage: g.build.damage * 1.45 * p, tick: i * .1, color: "#e5a85e", hostile: false });
  else if (["arrow-storm", "void-rain", "phantom-hunt"].includes(g.build.ultimate)) { if (g.build.ultimate === "phantom-hunt") g.shadowCloneTime = 7 * g.build.effectDuration; const aims: Point[] = g.phase === "boss" ? Array.from({ length: 40 }, () => g.boss) : g.enemies.slice(0, 65); aims.forEach((a, i) => projectile(g, g.player, Math.atan2(a.y - g.player.y, a.x - g.player.x) + (i % 3 - 1) * .025, 880, g.build.damage * (g.build.ultimate === "void-rain" ? 2.65 : 1.65) * p, false, { pierce: 4, life: 3.2 })); }
  else if (g.build.ultimate === "arcane-cataclysm") for (let i = 0; i < 8; i++) g.zones.push({ id: id(g), x: g.width * (.12 + (i % 4) * .25), y: g.height * (.27 + Math.floor(i / 4) * .48), radius: 170, life: 3, slow: .52, damage: g.build.damage * 1.15 * p, tick: i * .07, color: "#b39af7", hostile: false });
  else if (g.build.ultimate === "time-stop") { g.player.invulnerable = Math.max(g.player.invulnerable, 2.5); g.zones.push({ id: id(g), x: g.width / 2, y: g.height / 2, radius: whole, life: 7, slow: .9, damage: g.build.damage * .5 * p, tick: 0, color: "#8bc9f3", hostile: false }); }
  else if (g.build.ultimate === "singularity") { g.player.invulnerable = Math.max(g.player.invulnerable, 2.5); g.zones.push({ id: id(g), x: g.width / 2, y: g.height / 2, radius: whole * .62, life: 5.5, slow: .86, damage: g.build.damage * 1 * p, tick: 0, color: "#c48cff", hostile: false }); }
...
export function BossRush({ hero }: { hero: BossRushHero }) {
  ...
  useEffect(() => {
    ...
    const update = (dt: number) => {
      ...
      if (["horde", "boss"].includes(g.phase)) { g.attackClock -= dt; if (g.attackClock <= 0 && target(g)) { fireHero(g); g.attackClock = g.build.attackInterval; } if (g.build.fireballInterval) { g.fireballClock -= dt; if (g.fireballClock <= 0) { castTechnique(g); g.fireballClock = g.build.fireballInterval; } } if (g.build.rageInterval) { g.rageClock -= dt; if (g.rageClock <= 0) { g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: 150 * g.build.area, life: .82, slow: 0, damage: g.build.damage * 1.8, tick: 0, color: g.build.technique1God ? GODS[g.build.technique1God].color : "#e2a95e", hostile: false }); g.rageClock = g.build.rageInterval; } } if (g.build.slowFieldInterval) { g.slowFieldClock -= dt; if (g.slowFieldClock <= 0) { const fieldGod = g.build.technique1God ?? "frost"; g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: 135 * g.build.area, life: 5.4, slow: fieldGod === "frost" ? .65 : .45, damage: g.build.damage * (fieldGod === "frost" ? .17 : .23), tick: 0, color: GODS[fieldGod].color, hostile: false, god: fieldGod }); g.slowFieldClock = g.build.slowFieldInterval; } } if (g.build.novaInterval) { g.novaClock -= dt; if (g.novaClock <= 0) { const god = g.build.technique1God ?? "moon"; g.zones.push({ id: id(g), x: g.player.x, y: g.player.y, radius: 180 * g.build.area, life: .9, slow: god === "shadow" ? .4 : .15, damage: g.build.damage * 1.65, tick: 0, color: GODS[god].color, hostile: false, god }); if (god === "blood") for (const e of g.enemies) if (distance(e, g.player) < 185 * g.build.area) bleed(g, e, g.build, 2); g.novaClock = g.build.novaInterval; } } }
      if (g.build.trapInterval) { g.trapClock -= dt; if (g.trapClock <= 0) { const angle = Math.random() * Math.PI * 2, radius = 70 + Math.random() * 170; g.zones.push({ id: id(g), x: clamp(g.player.x + Math.cos(angle) * radius, 40, g.width - 40), y: clamp(g.player.y + Math.sin(angle) * radius, 40, g.height - 40), radius: 60 + g.build.trapDamage * 3.5, life: 12, slow: .4, damage: g.build.damage * Math.max(.9, g.build.trapDamage), tick: .22, color: GODS.stone.color, hostile: false, kind: "trap", armed: .7, god: "stone" }); g.trapClock = g.build.trapInterval; } }
      ...
      while (g.summons.length < g.build.summonCount) { const index = g.summons.length, hp = Math.max(80, g.build.summonHp), summonGod = g.build.summonGod ?? "shadow"; g.summons.push({ id: id(g), x: g.player.x, y: g.player.y, hp, maxHp: hp, damage: g.build.damage * Math.max(.45, g.build.summonDamage), radius: 13, cooldown: .25 + index * .1, color: GODS[summonGod].color, god: summonGod, orbit: index * Math.PI * 2 / Math.max(1, g.build.summonCount) }); }
      for (const summon of g.summons) { const orbit = 105 + Math.sin(g.elapsed * .7 + summon.id) * 15, desired = { x: g.player.x + Math.cos(g.elapsed * .55 + summon.orbit) * orbit, y: g.player.y + Math.sin(g.elapsed * .55 + summon.orbit) * orbit }; summon.x += (desired.x - summon.x) * Math.min(1, dt * 4); summon.y += (desired.y - summon.y) * Math.min(1, dt * 4); summon.cooldown -= dt; if (summon.cooldown <= 0) { const victim = g.enemies.filter(e => e.hp > 0).sort((a, b) => distance(summon, a) - distance(summon, b))[0] ?? (g.phase === "boss" ? g.boss : null); if (victim && distance(summon, victim) < 420) projectile(g, summon, Math.atan2(victim.y - summon.y, victim.x - summon.x), 540, summon.damage, false, { radius: 5, pierce: 1, life: 1.25, color: summon.color, shape: "shard", god: summon.god, special: "summon" }); summon.cooldown = 1; } } g.summons = g.summons.filter(s => s.hp > 0);
      if (g.build.orbitals > 0) { const orbitalGod = g.build.technique2God, color = orbitalGod ? GODS[orbitalGod].color : g.build.accent, hitRadius = 20 + Math.min(16, Math.max(0, g.build.orbitalRadius - 84) * .1), orbitSpeed = orbitalGod === "gale" || orbitalGod === "storm" ? 1.85 : orbitalGod === "stone" ? .95 : 1.35; for (let i = 0; i < g.build.orbitals; i++) { const angle = g.elapsed * (orbitSpeed + i % 2 * .13) + i * Math.PI * 2 / g.build.orbitals, orbit = g.build.orbitalRadius + Math.sin(g.elapsed * 2 + i) * 9, point = { x: g.player.x + Math.cos(angle) * orbit, y: g.player.y + Math.sin(angle) * orbit }, damage = g.build.damage * g.build.orbitalDamage * dt * 4.5; for (const e of g.enemies) if (distance(point, e) < e.radius + hitRadius) touchOrbital(g, e, damage, orbitalGod, dt); if (g.phase === "boss" && g.boss.vulnerable && distance(point, g.boss) < g.boss.radius + hitRadius) { touchOrbital(g, g.boss, damage, orbitalGod, dt); charge(g, damage); } if (orbitalGod === "venom" && Math.random() < dt * .9) g.zones.push({ id: id(g), ...point, radius: 26, life: .6, slow: .15, damage: g.build.damage * .1, tick: .1, color, hostile: false, god: "venom" }); } }
